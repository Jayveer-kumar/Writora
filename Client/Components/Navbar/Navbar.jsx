  ...
  return (
    <nav className="sticky top-0 z-50 flex h-16 items-center justify-between border-b  border-brand-border/10 bg-brand-bg px-4 md:px-8 transition-colors duration-300">
      ...
      <div className="flex items-center gap-2 md:gap-5">
        <button
          onClick={toggleMode}
          className=" darkModeButton  cursor-pointer p-2  text-brand-text hover:text-brand-hover rounded-full relative"
        >
        ...
        <div
          onClick={(e) => openDropdown.current.toggle(e)}
          className="h-8 w-8 rounded-full bg-blue-600 flex items-center justify-center text-white cursor-pointer overflow-hidden border border-brand-border"
        >
          {user?.avatar ? (
